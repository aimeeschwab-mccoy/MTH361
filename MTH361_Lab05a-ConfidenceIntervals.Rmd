---
title: 'Lab 5a: Confidence Intervals'
...
The `t.test()` command can be used to calculate confidence intervals. For example, the command to calculate a 95\% confidence interval for `height` in `yrbss.complete` is

```{r, eval = FALSE}
t.test(yrbss.complete$height, conf.level = 0.95)
```
